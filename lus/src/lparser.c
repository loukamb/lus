...
static void discharge2basereg(FuncState *fs, expdesc *e, int basereg) {
  ...
  if (e->k == VRELOC) {
    ...
    switch (e->k) {
      case VNIL: luaK_codeABC(fs, OP_LOADNIL, basereg, 0, 0); break;
      case VFALSE: luaK_codeABC(fs, OP_LOADFALSE, basereg, 0, 0); break;
      case VTRUE: luaK_codeABC(fs, OP_LOADTRUE, basereg, 0, 0); break;
      default:
...
static void suffixedexp(LexState *ls, expdesc *v) {
  /* suffixedexp ->
     ...
     Key: all operations in an optional chain write to the SAME register (basereg).
     When we jump (value is nil), the register already contains nil, so the
     result is correct without needing to explicitly load nil. */
  FuncState *fs = ls->fs;
...
static void simpleexp(LexState *ls, expdesc *v) {
  ...
  switch (ls->t.token) {
    ...
    case TK_CATCH: { /* catch expression */ catchexpr(ls, v); return;
    }
    case TK_ENUM: { /* enum expression */ enumexpr(ls, v); return;
    }
    default: {
      suffixedexp(ls, v);
      return;
...
static void catchexpr(LexState *ls, expdesc *v) {
  ...
  /* Emit OP_ENDCATCH: A=base, B=nresults (default 2), C=jump offset (placeholder)
...
static void test_then_block(LexState *ls, int *escapelist) {
  ...
  if (isassigncond(ls)) {
    ...
    condexit = cond(ls); /* read normal condition */
  }
...
** 'local a, b, c from tbl' is equivalent to 'local a, b, c = tbl.a, tbl.b, tbl.c'
...
static void localstat(LexState *ls) {
  ...
  int firstidx;     /* index of first variable */
  int nvars = 0;
  int nexps;
  expdesc e;
...
static lu_byte getglobalattribute(LexState *ls, lu_byte df) {
  ...
  switch (kind) {
    ...
    case RDKCONST: return GDKCONST; /* adjust kind for global variable */
    default: return kind;
  }
}
...
** 'global a, b, c from tbl' is equivalent to 'global a, b, c = tbl.a, tbl.b, tbl.c'
...
static void statement(LexState *ls) {
  ...
  switch (ls->t.token) {
    ...
    case TK_BREAK: { /* stat -> breakstat */ breakstat(ls, line); break;
    }
