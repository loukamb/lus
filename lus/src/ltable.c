...
static int equalkey(const TValue *k1, const Node *n2, int deadok) {
  if (rawtt(k1) != keytt(n2)) { /* not the same variants? */
    ...
    switch (keytt(n2)) {
      ...
      case ctb(LUA_VLNGSTR): return luaS_eqstr(tsvalue(k1), keystrval(n2));
...
lu_byte luaH_get(Table *t, const TValue *key, TValue *res) {
  const TValue *slot;
  switch (ttypetag(key)) {
    case LUA_VSHRSTR: slot = luaH_Hgetshortstr(t, tsvalue(key)); break;
    case LUA_VNUMINT: return luaH_getint(t, ivalue(key), res);
    case LUA_VNIL: slot = &absentkey; break;
    case LUA_VNUMFLT: {
      lua_Integer k;
    ...
    default: slot = getgeneric(t, key, 0); break;
  }
...
int luaH_pset(Table *t, const TValue *key, TValue *val) {
  switch (ttypetag(key)) {
    ...
    default: return finishnodeset(t, getgeneric(t, key, 0), val);
  }
}
