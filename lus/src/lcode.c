...
void luaK_setreturns(FuncState *fs, expdesc *e, int nresults) {
  ...
  if (e->k == VCALL) /* expression is an open function call? */
    ...
    /* Catch is a multi-return expression: (status, ...inner_results).
    ** e->u.info points to the ENDCATCH instruction.
    ** Set its B field to the requested nresults for proper nil-filling. */
...
void luaK_setoneret(FuncState *fs, expdesc *e) {
  if (e->k == VCALL) { /* expression is an open function call? */
    ...
    ** e->u.info points to ENDCATCH instruction; get base reg from A field. */
    e->k = VNONRELOC; /* result has fixed position */
    e->u.info = GETARG_A(getinstruction(fs, e));
  }
}
...
void luaK_indexed(FuncState *fs, expdesc *t, expdesc *k) {
  ...
  if (k->k == VKSTR)
    keystr = str2K(fs, k);
  lua_assert(!hasjumps(t) && (t->k == VLOCAL || t->k == VVARGVAR ||
