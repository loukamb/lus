...
static l_mem traversetable(global_State *g, Table *h) {
  ...
  switch (getmode(g, h)) {
    case 0: /* not weak */ traversestrongtable(g, h); break;
    case 1: /* weak values */ traverseweakvalue(g, h); break;
    case 2: /* weak keys */ traverseephemeron(g, h, 0); break;
...
void luaC_step(lua_State *L) {
  ...
  if (!gcrunning(g)) {       /* not running? */
    ...
    switch (g->gckind) {
      case KGC_INC:
      case KGC_GENMAJOR: incstep(L, g); break;
      case KGC_GENMINOR:
        youngcollection(L, g);
        setminordebt(g);
        break;
